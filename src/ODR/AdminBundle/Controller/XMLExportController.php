...
//                $response->sendHeaders();
     ...
     * @return Response
     */
    public function getDatarecordXMLAction($version, $datarecord_id, Request $request)
    {
        $return = array();
        $return['r'] = 0;
        $return['t'] = '';
        ...
        try {
            ...
            $xml = self::GetDisplayData($em, $version, $datarecord_id, $request);

            // Set up a response to send the file back
            $response->setPrivate();
            $response->headers->set('Content-Type', 'text/xml');
            //$response->headers->set('Content-Length', filesize($xml_export_path.$filename));
            $response->headers->set('Content-Disposition', 'attachment; filename="Datarecord_'.$datarecord_id.'.xml";');
...
//print '<pre>'.print_r($datatype_array, true).'</pre>';  exit();
            ...
            $template = 'ODRAdminBundle:XMLExport:xml_datarecord_ajax.html.twig';
